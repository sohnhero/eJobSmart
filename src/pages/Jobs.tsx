            ...
            {filtered.length === 0 ? (
              ...
              <div className={viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-4' : 'space-y-3'}>
                {filtered.map(job => (
                  <div
                    key={job.id}
                    className={`card p-5 cursor-pointer hover:-translate-y-0.5 transition-all duration-200 group relative ${viewMode === 'list' ? 'flex items-start gap-4' : ''}`}
                    onClick={() => navigate(`/jobs/${job.id}`)}
                  >
                    {/* Badges */}
                    <div className="absolute top-3 right-3 flex gap-1.5">
                      {job.isBoosted && <Badge variant="amber" size="sm"><Zap className="w-2.5 h-2.5" /></Badge>}
                      <button
                      ...
                      >
                        <Bookmark className={`w-4 h-4 ${savedJobs.has(job.id) ? 'fill-brand-600 text-brand-600' : 'text-slate-300 hover:text-brand-400'}`} />
                      </button>
                    </div>

                    {viewMode === 'list' ? (
                      <>
                        <img src={job.companyLogo} alt={job.company} className="w-12 h-12 rounded-xl object-cover flex-shrink-0 mt-1" />
                        <div className="flex-1 min-w-0 pr-16">
                          <h3 className="font-semibold text-slate-900 text-sm group-hover:text-brand-600 transition-colors">{job.title}</h3>
                          <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1"><Building2 className="w-3 h-3" />{job.company}</p>
                          <div className="flex flex-wrap gap-1.5 mt-2">
                            <ContractBadge type={job.contractType} />
                            <Badge variant="slate">{job.remoteType}</Badge>
                            <Badge variant="slate"><MapPin className="w-2.5 h-2.5" />{job.city}</Badge>
                          </div>
                      ...
                      <>
                        <div className="flex items-start gap-3 mb-4 pr-16">
                          ...
                          <div className="min-w-0">
                            <h3 className="font-semibold text-slate-900 text-sm leading-tight line-clamp-2 group-hover:text-brand-600 transition-colors">{job.title}</h3>
                            <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1"><Building2 className="w-3 h-3" />{job.company}</p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-500 mb-3">
                          ...
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{new Date(job.postedAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
                        </div>
